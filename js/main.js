...
$(document).ready(function() {
  ...
  // Check the current page URL and add the 'menu-active' class to the corresponding navigation menu item
  var currentPageUrl = window.location.href;
  $('.nav-menu a').each(function() {
    var menuItemHref = $(this).attr('href');
    if (currentPageUrl.indexOf(menuItemHref) > -1) {
      $('.nav-menu li').removeClass('menu-active');
      $(this).closest('li').addClass('menu-active');
    }
  });
...
// read more in about us (dialog)
$(document).ready(function() {
            $(".read-more a").on("click", function(e) {
                e.preventDefault();
                var paragraph = $(this).closest(".about-col").find("p").text();
                $("#dialog .dialog-content").text(paragraph);
                $("#dialog").fadeIn();
            });
